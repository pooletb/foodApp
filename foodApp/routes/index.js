...
.then((result) => {
  premade_food = result;
  for(let i = 0; i < premade_food.length; i++) {
    var containsAllergens = [];
    Promise.all(knex('premade_food').where('premade_food.food_ID', premade_food[i].food_ID).innerJoin('premade_contains', 'premade_food.food_ID', 'premade_contains.food_ID'))
